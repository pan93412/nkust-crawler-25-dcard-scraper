...
export default class Storer {
	...
	async storeComment(
		...
		console.log(
			`Attempting to store comment: ${comment.id} for article: ${articleId}`,
		);
		try {
			const response = await fetch(
				`${this.#endpoint}/articles/${articleId}/comments`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
			...
			if (!response.ok) {
				console.error(
					`Failed to store comment ${comment.id}: ${response.statusText}`,
				);
				return false;
			}
	...
	async storeReply(
		...
		console.log(
			`Attempting to store reply: ${reply.id} for comment: ${commentId}`,
		);
		try {
			const response = await fetch(
			...
			if (!response.ok) {
				console.error(
					`Failed to store reply ${reply.id}: ${response.statusText}`,
				);
				return false;
			}
